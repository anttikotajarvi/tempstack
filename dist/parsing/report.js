...
function classifyDirType(name, cfg) {
    ...
    if (name.startsWith(cfg.groupDirPrefix))
        return "group-dir";
    if (name.startsWith(cfg.HIDDEN_DIR_PREFIX))
        return "hidden-dir";
    return "anchor-dir";
}
/**
...
function computeAnchor(localPath, cfg) {
    ...
    return withoutLast.filter((seg) => !seg.startsWith(cfg.groupDirPrefix) &&
...
function buildDirNode(cfg, name, parentLocalPath, parentFsPath) {
    ...
    const dirAnchor = dirType === "anchor-dir" ? [name] : []; // root/group/hidden contribute nothing
    const node = {
        nodeType: "dir",
        name,
        localPath,
    ...
    for (const entry of entries) {
        ...
        if (ext === cfg.LITERAL_EXT) {
            ...
            const fnSrc = (() => {
                const src = node_fs_1.default.readFileSync(fileFsPath, "utf8");
                return src;
            })();
