...
The resolved JsonValue is pushed _as-is_ (no flattening):
...
Here’s a concise README section you can drop in as-is. I’ve written it so it explains **what**, **when**, and **how**, without going into implementation details or edge-case rambling.

---

## Overrides and Patches

Tempstack supports **two argument-based mechanisms** for modifying rendered output without changing templates: **`__override`** and **`__patch`**. They serve different purposes and have intentionally different semantics.

### `__override` — Object-level overrides (merge last)

Use `__override` when you want to **replace or extend parts of the final object**, regardless of whether those fields already exist.

- Applied **after all templates**
- **Deep-merges** objects
- **Replaces arrays** (arrays are atomic here)
- Can **add new fields**
- Safe and flexible — recommended for most UI-driven overrides

**Example:**

```js
render(["config/.a/feature"], {
  __override: {
    config: {
      mode: "override",
      flags: {
...
### `__patch` — Targeted value replacement (strict)

Use `__patch` when you want to **surgically modify existing values** and be warned if the target does not exist.

- Applied **after all templates**
- **Replaces the value at an exact path**
- **Throws if the path does not exist**
- Intended for **atomic values**, not structural changes
- Order matters: patches are applied sequentially

`__patch` is an array of `[path, value]` entries:

```js
render(["version/old"], {
  __patch: [["version", "0.0.2"]],
});
```
...
Setting a patch value to `undefined` removes the field from the final output:
...
Array paths use the same `[]` semantics as templates.

- Patching an array **element**:

```js
__patch: [["array-tests/colors/[]", "red"]];
```

- Multiple pushes require multiple patch entries:

```js
__patch: [
  ["array-tests/colors/[]", "red"],
  ["array-tests/colors/[]", "blue"],
];
```
